...
if __name__ == "__main__":
        ...
        for month in months:
            if year != "2016" or month != "12":
                ...
                data.append(
                    ...
                    .load(
                        f"/user/hadoop/hubway/kpis/{year + month}-hubway-tripdata.csv"
                    )
                )
    ...
    # Write data to HDFS
    kpi_data.write.format("csv").option("header", "true").mode("overwrite").save(
        f"/user/hadoop/hubway/kpis/overall_kpis.csv"
    )
