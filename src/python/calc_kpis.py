...
if __name__ == '__main__':
    ...
    for gender in ["Other", "Male", "Female"]:
        gender_shares = gender_shares.withColumnRenamed(f"count(CASE WHEN (gender = {gender}) THEN true END)",gender)

        gender_shares = gender_shares.withColumn(gender, F.col(gender) / final_data.count())

        kpis = kpis.withColumn(f"gender {gender} percentage", lit(f"{gender_shares.collect()[0][i]}"))
        i += 1
    ...
    for generation in ["Silent Generation", "Baby Boomers", "Generation X", "Millenials", "Zoomers", "Not given"]:
        generation_shares = generation_shares.withColumnRenamed(f"count(CASE WHEN (generation = {generation}) THEN true END)",generation)

        generation_shares = generation_shares.withColumn(generation, F.col(generation) / final_data.count())
        kpis = kpis.withColumn(f"generation {generation} percentage", lit(f"{generation_shares.collect()[0][i]}"))
        i += 1
    ...
    for timeslot in ["0","1","2","3"]:
        timeslot_shares = timeslot_shares.withColumnRenamed(f"count(CASE WHEN (timeslot = {timeslot}) THEN true END)",timeslot)

        timeslot_shares = timeslot_shares.withColumn(timeslot, F.col(timeslot) / final_data.count())
        kpis = kpis.withColumn(f"timeslot {int(timeslot) * 6} till {int(timeslot) * 6 + 6} percentage", lit(f"{timeslot_shares.collect()[0][i]}"))
        i += 1
    
    top_bikes = ""
    i = 1
    for row in final_data.groupBy('bikeid').count().sort(col("count").desc()).limit(10).collect():
        if i == 10:
    ...
    i = 1
    for row in final_data.groupBy('start station id').count().sort(col("count").desc()).limit(10).collect():
        if i == 10:
    ...
    i = 1
    for row in final_data.groupBy('start station name').count().sort(col("count").desc()).limit(10).collect():
        if i == 10:
    ...
    top_end_stationids = ""
    i = 1
    for row in final_data.groupBy('end station id').count().sort(col("count").desc()).limit(10).collect():
        if i == 10:
    ...
    i = 1
    for row in final_data.groupBy('end station name').count().sort(col("count").desc()).limit(10).collect():
        if i == 10:
    ...
    # Write data to HDFS
    kpis.write.format('csv').option("header", "true").\
            mode('overwrite').\
        save(f"/user/hadoop/hubway/kpis/{yearmonth}-hubway-tripdata.csv")