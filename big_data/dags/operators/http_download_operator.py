...
class HttpDownloadOperator(BaseOperator):
    ...
    @apply_defaults
    def __init__(self, download_uri, save_to, *args, **kwargs):
        """
    ...
    def execute(self, context):
        ...
        self.log.info(
            "Downloading '" + self.download_uri + "' to '" + self.save_to + "'."
        )
        ...
        try:
            ...
            raise AirflowException(
                "Failure, could not execute request. Exception: " + str(e)
            )

        if r.status_code != 200:
            raise AirflowException(
                "Failure, could not download file. HTTP Code: " + str(r.status_code)
            )
